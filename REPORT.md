...
### 7.1 배포 환경
- **개발 환경**: Next.js 개발 서버 (API 라우트 사용 가능)
- **프로덕션 환경**: GitHub Pages (정적 사이트, localStorage 사용)

### 7.2 호환성 처리
- API 사용 가능 시: 서버 메모리 + localStorage 이중 저장
- API 사용 불가 시: localStorage만 사용하여 정상 작동
- 에러 처리: API 실패 시 조용히 폴백하여 사용자 경험 유지

### 7.3 빌드 최적화
- 정적 사이트 빌드를 위한 API 라우트 제외 스크립트
...
### 9.2 제한사항
1. **서버 메모리 저장**: 서버 재시작 시 데이터 손실 (개발 환경)
2. **정적 사이트 제약**: GitHub Pages 배포 시 API 미사용
3. **브라우저 의존성**: localStorage 사용으로 브라우저별 저장 용량 제한

## 10. 향후 개선 방향

1. **데이터베이스 연동**: 영구 저장을 위한 DB 연동 (PostgreSQL, MongoDB 등)
2. **실시간 대시보드**: WebSocket을 통한 실시간 데이터 업데이트
3. **고급 분석**: 머신러닝 기반 이상 행동 탐지
4. **다중 테스트**: 동시에 여러 A/B 테스트 실행 지원
5. **데이터 내보내기**: CSV/JSON 형식으로 데이터 내보내기 기능
